...
recurse:
    ...
    # THINK: why a0?
    # a0 the address of the first node , s0 store &node, a0 = node.value
    # 4(s0) is the &(node->next)
    ### YOUR CODE HERE ###
    lw a0, 0(s0)

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why).
    # What function? Recall the parameters of "map"
    # because the map function may change, so we can not use a specified label, we use s1
    ### YOUR CODE HERE ###
    jalr ra, s1, 0
